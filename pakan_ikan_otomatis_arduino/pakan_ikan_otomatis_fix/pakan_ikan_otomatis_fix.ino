...
rtc.begin();
rtc.setDOW(MONDAY);     // Set Day-of-Week to SUNDAmy
rtc.setTime(21, 58, 0);     // Set the time to 12:00:00 (24hr format)
rtc.setDate(23, 9, 2024);   // Set the date to January 1st, 2014
...
if(kolam < 50){
 ...
 adcPH = analogRead(A1); //menggunakan pin A0 untuk membaca output sensor pH
...
void beep(int kedip) {
   for (int i = 0; i < kedip; i++) {
    ...
    delay(500);             // Tunggu sesuai durasi kedip
    digitalWrite(buzzer, LOW);   // Matikan buzzer
    delay(800);             // Tunggu sebelum kedip berikutnya
  }
}
